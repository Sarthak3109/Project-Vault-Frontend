...
const ProjectCard = ({
    ...
    return (
        <div className="w-[400px] mt-[30px] mb-2 sm:mt-[2px] lg:w-[300px] rounded-lg shadow-md  hover:translate-y-1">
        ...
        {/* <ToastContainer /> */}
             ...
             {  LoggedInUser === User?._id && <div className="flex justify-center items-center gap-4">
                <AiFillEdit size = "20px" className='cursor-pointer text-[red]' onClick={()=>{navigate(`/posts/edit/${id}`)}} /><AiFillDelete size = "20px"
             onClick = {()=>{

                    if(window.confirm("are you sure you want to delete this post")){
                        DeletePostById(id).then(()=>toast("POST DELETED SUCCESSFULLY, REFRESH TO SEE CHANGES"))
                    }
                    else{
                        return;
                    }
